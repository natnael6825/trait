...
async function handleCheckoutSessionCompleted(supabaseClient: any, event: any) {
  ...
  try {
    ...
    if (subscriptionError) {
      console.error("Error fetching subscription ID:", subscriptionError);
      throw new Error(
        `Failed to fetch subscription ID: ${subscriptionError.message}`,
      );
    }

    // Call the update-credits function to add credits based on the subscription
    try {
      const updateCreditsResponse = await fetch(
        `${Deno.env.get("SUPABASE_URL")}/functions/v1/update-credits`,
        {
          method: "POST",
          headers: {
      ...
      const updateCreditsResult = await updateCreditsResponse.json();
      console.log("Update credits result:", updateCreditsResult);

      if (!updateCreditsResponse.ok) {
        console.error("Error updating credits:", updateCreditsResult);
      }
    } catch (creditsError) {
      console.error("Error calling update-credits function:", creditsError);
